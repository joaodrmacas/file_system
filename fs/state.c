...
pthread_mutex_t free_open_file_mutex;
...
int state_init(tfs_params params) {
    ...
    inode_rwl_table = malloc(INODE_TABLE_SIZE* sizeof(pthread_rwlock_t));
    ...
    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        freeinode_ts[i] = FREE;
        pthread_rwlock_init(&inode_rwl_table[i],NULL);
    }

    pthread_rwlock_init(&freeinode_rwl,NULL);

    for (size_t i = 0; i < DATA_BLOCKS; i++) {
        free_blocks[i] = FREE;
    }
    pthread_rwlock_init(&free_block_rwl,NULL);

    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_mutex_init(&open_file_table[i].mutex,NULL);
        free_open_file_entries[i] = FREE;
    }

    pthread_mutex_init(&free_open_file_mutex,NULL);
...
int state_destroy(void) {
    ...
    for (int i=0; i<INODE_TABLE_SIZE; i++){
        pthread_rwlock_destroy(&inode_rwl_table[i]);
    }

    pthread_rwlock_destroy(&freeinode_rwl);
    pthread_rwlock_destroy(&free_block_rwl);

    for (int i=0;i<MAX_OPEN_FILES;i++)
        pthread_mutex_destroy(&open_file_table[i].mutex);

    pthread_mutex_destroy(&free_open_file_mutex);

    free(inode_table);
    free(inode_rwl_table);
    free(freeinode_ts);
    free(fs_data);
    free(free_blocks);
 ...
 */
static int inode_alloc(void) { //Posso fazer isto?
    pthread_rwlock_wrlock(&freeinode_rwl);
    for (size_t inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            pthread_rwlock_unlock(&freeinode_rwl);
            return (int)inumber;
        }
    }
    pthread_rwlock_unlock(&freeinode_rwl);
...
int inode_create(inode_type i_type) {
    ...
    //preciso de lock?
    pthread_rwlock_rdlock(&freeinode_rwl);
    inode_t *inode = &inode_table[inumber];
    insert_delay(); // simulate storage access delay (to inode)

    // pthread_rwlock_wrlock(&inode_rwl_table[inumber]);
    inode->i_node_type = i_type;
    // pthread_rwlock_unlock(&inode_rwl_table[inumber]);
    ...
    case T_DIRECTORY: {
        ...
        if (b == -1) {
            ...
            inode->i_data.i_data_block = -1;
            inode->i_data.i_links_count = 0;
        ...
        inode_table[inumber].i_data.i_size = BLOCK_SIZE;
        inode_table[inumber].i_data.i_data_block = b;
        inode_table[inumber].i_data.i_links_count = 1;
    ...
    case T_FILE:
        ...
        inode_table[inumber].i_data.i_size = 0;
        inode_table[inumber].i_data.i_data_block = -1;
        inode_table[inumber].i_data.i_links_count = 1;
        // pthread_rwlock_unlock(&inode_rwl_table[inumber]);
        break;
    case T_SYMLINK: {
        // pthread_rwlock_wrlock(&inode_rwl_table[inumber]);
        memset(inode_table[inumber].i_symlink_name,0,MAX_FILE_NAME);
        // pthread_rwlock_unlock(&inode_rwl_table[inumber]);
    } break;
    default:
...
void inode_delete(int inumber) {
    ...
    if (inode_table[inumber].i_node_type!=T_SYMLINK && inode_table[inumber].i_data.i_size > 0) {
        data_block_free(inode_table[inumber].i_data.i_data_block);
    }
...
int clear_dir_entry(inode_t *inode, char const *sub_name) {
    ...
    pthread_rwlock_wrlock(&inode_rwl_table[ROOT_DIR_INUM]);
    
    // Locates the block containing the entries of the directory
    dir_entry_t *dir_entry = (dir_entry_t *)data_block_get(inode->i_data.i_data_block);
    ALWAYS_ASSERT(dir_entry != NULL,
    ...
    for (size_t i = 0; i < MAX_DIR_ENTRIES; i++) {
        if (!strcmp(dir_entry[i].d_name, sub_name)) {
            ...
            pthread_rwlock_unlock(&inode_rwl_table[ROOT_DIR_INUM]);
            return 0;
        }
    }
    pthread_rwlock_unlock(&inode_rwl_table[ROOT_DIR_INUM]);
...
int add_dir_entry(inode_t *inode, char const *sub_name, int sub_inumber) {
    ...
    if (find_in_dir(inode,sub_name) != -1){
        fprintf(stderr,"add_dir_entry error: %s already exists.\n", sub_name);
        return -1;
    }
...
int find_in_dir(inode_t const *inode, char const *sub_name) {
    ...
    pthread_rwlock_rdlock(&inode_rwl_table[ROOT_DIR_INUM]);
...
int data_block_alloc(void) {
    pthread_rwlock_rdlock(&free_block_rwl);
    for (size_t i = 0; i < DATA_BLOCKS; i++) {
        if (i * sizeof(allocation_state_t) % BLOCK_SIZE == 0) {
            insert_delay(); // simulate storage access delay to free_blocks
        }
        pthread_rwlock_unlock(&free_block_rwl);
        pthread_rwlock_wrlock(&free_block_rwl);
...
int add_to_open_file_table(int inumber, size_t offset) {
    pthread_mutex_lock(&free_open_file_mutex);
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        if (free_open_file_entries[i] == FREE) {
            ...
            pthread_mutex_unlock(&free_open_file_mutex);
            return i;
        }
    }
    pthread_mutex_unlock(&free_open_file_mutex);
